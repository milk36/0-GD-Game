...
| [voxel-to-tile.html](voxel-to-tile.html) | 体素改瓦片：方案评估与建议（「瓦片」四种含义适配度/为何不退 2D TileMapLayer/三层架构·瓦片本体+排布层+渲染层/滚动走廊行式瓦片/必须先定死的三个数·边长·锚点·北向/渲染层选型 GridMap·MultiMesh·合并 mesh/性能账约 87→15~25 次绘制/与 spec 管线的复用边界·共用 op 实现另立 terrain schema/阶段 1~4 落地路径·定规格→海面→pirate 试点→现有地貌/风险边界/待定决策清单；**当前状态：仅方案，未开工**） |
