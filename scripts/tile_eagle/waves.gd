...
const ENEMY := {
	...
	"E16": {
		# 野牛气垫登陆艇（22×30×11 = 6.6×9.0×3.3，全表最宽最厚）：LOW 层缓进，
		# 扇形齐射 ↔ 环形弹幕交替（fortress）。**击沉时放出搭载的 E1/E2**（def.drop，
		# combat.gd::_damage）—— 登陆艇的戏就是「沉了也要把兵送上岛」
		"vox": "E16", "hp": 26, "layer": "LOW", "beh": "fortress",
		"hit": Vector2(3.3, 4.5), "score": 450, "stars": 5,
		"gun": "aimed", "n": 4, "spread": 24.0, "bspeed": 6.5, "cd": 2.6,
		"ring_n": 12, "ring_spd": 5.5, "drop": ["E1", "E2"],
		"vz": 1.8, "range_z": Vector2(-40.0, 8.0),
	},
	"E14": {
		# 零式战斗机·红涂装（13×15×5 = 3.9×4.5×1.5，全表最小）：AIR 层大振幅侧摆（sway 3.5，
		# 常规机 1.5）—— 小、快、摆得多，靠 def.sway 与 E4 区分
		"vox": "E14", "hp": 3, "layer": "AIR", "beh": "weave", "sway": 3.5,
		"hit": Vector2(1.2, 1.5), "score": 120, "stars": 1,
		"gun": "aimed", "n": 1, "spread": 0.0, "bspeed": 8.5, "cd": 2.0,
		"vz": 9.5, "range_z": Vector2(-44.0, 40.0),
	},
	"E15": {
		# 零式战斗机·金涂装：与 E14 同机体异涂装（金黄），双管点射节奏更慢
		"vox": "E15", "hp": 3, "layer": "AIR", "beh": "weave", "sway": 3.5,
		"hit": Vector2(1.2, 1.5), "score": 120, "stars": 1,
		"gun": "aimed", "n": 2, "spread": 10.0, "bspeed": 8.0, "cd": 2.4,
		"vz": 9.5, "range_z": Vector2(-44.0, 40.0),
	},
	"E8": {
...
const WAVES := [
	...
	{"t": 176.0, "spawn": [{"t": "E14", "x": -7}, {"t": "E15", "x": 7}]},           # 零式双机（红/金中队）
	{"t": 186.0, "spawn": [{"t": "E6", "x": -4.0}, {"t": "E5", "x": 9.0}]},          # 精英炮舰
	{"t": 212.0, "spawn": [{"t": "E10", "x": 0.0}]},                                # 浮空盾堡
	{"t": 222.0, "spawn": [{"t": "E16", "x": 0.0}, {"t": "E4", "x": -10.0},
			{"t": "E4", "x": 10.0}]},                                    # 野牛登陆突击（击沉放兵）
	{"t": 232.0, "spawn": [
