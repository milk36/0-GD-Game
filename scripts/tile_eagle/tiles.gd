...
const HIGH_IDS: Array[String] = ["reef_s", "island_4x4", "island_2x2", "sandbar_2x2"]
## 多格跨度的「超级瓦」：一张资产占 N×N 格。尺寸三档 = 岛 19.2 / 小岛 9.6 / 沙洲 9.6（世界单位）。
...
## 水面 0.30 / 沙滩 0.60 / 草丘 0.90~1.50。这条恒等式是地面单位贴地的依据。
##
## 产出：
## - `heights`  PackedFloat32Array，下标 `(z−mn.z)·sx + (x−mn.x)`，值 = 该列顶面的世界 y
## - `shore`    沙面（世界 y ≤ 0.75）的列，元素是**网格局部坐标**的列顶中心
...
static func _fill_columns(d: Dictionary, blocks: Dictionary) -> void:
	...
	for key in top:
		...
		if world_y > Altitude.SEA + 0.15 and world_y <= Altitude.SEA + 0.45:
			shore.append(Vector3(
